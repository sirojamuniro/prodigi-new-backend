...
class ManajemenViewProductController {
	static async listView(req, res) {
        let listWishlist = []
        let no = 1;
        try{
            let countProduct = await ViewProduct.findAll({
				include:[{
					model:Product
				}],
                raw : true ,
            ...
            res.render('viewproduct', {
                title: 'List View Product',
                list_view_product: listWishlist,
                list_view_active: 'active'
            });
    
        }
        catch (error) {
        req.flash('msg_error', error.message || `Eror mendapatkan viewproduct`);
        res.redirect('/admin/listviewproduct')}
...
module.exports = ManajemenViewProductController